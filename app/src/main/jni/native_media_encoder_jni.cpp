...
std::shared_ptr<ANativeWindow> NativeEncoder::getPersistentSurface() {
    ...
    if (ret != AMEDIA_OK) {
        LOGE("Failed to create persistent surface !");
        return nullptr;
    }
    LOGI("Encoder: created persistent surface %p", ps);
...
Status NativeEncoder::prepare(
    ...
    LOGI("NativeEncoder::prepare");
    mRunConfig = std::move(runConfig);

    AMediaFormat *config = mRunConfig->format();
    LOGI("Encoder format: %s", AMediaFormat_toString(config));
    ...
    if (surface == nullptr) {
        ...
        LOGI("created input surface = %p", mLocalSurface.get());
    } else {
        LOGI("setting persistent input surface %p", surface.get());
    ...
    while (nFrameCount < framesToEncode) {
        ...
        for (;paramItr != dynamicParams.end()
            ...
            if (p->frameNum() == nFrameCount) {
                ...
                LOGI("Encoder DynamicParam @frame[%d] - applying setting : %s",
                        nFrameCount, s);
        ...
        if (status >= 0) {
            LOGI("got encoded buffer[%d] of size=%d @%lld us flags=%x",
            ...
            LOGI("format changed: %s", AMediaFormat_toString(format.get()));
        } else if (status == AMEDIACODEC_INFO_TRY_AGAIN_LATER) {
        } else {
            LOGI("Invalid status : %d", status);
        }
    }
