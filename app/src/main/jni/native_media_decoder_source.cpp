...
Status DecoderSource::setDataSourceFd(
    ...
    for (int i = 0; i < numtracks; i++) {
        ...
        if (!AMediaFormat_getString(format, AMEDIAFORMAT_KEY_MIME, &mime)) {
            ...
            LOGE("expected video mime type, got %s", mime);
            mEx = nullptr;
        }
        AMediaFormat_delete(format);
    }
...
void DecoderSource::run() {
    while(!mStopRequest) {
        ...
        if (status >= 0) {
            LOGI("got decoded buffer of size=%d @%lld us",
            ...
            LOGI("format changed: %s", AMediaFormat_toString(mFormat.get()));
...
Status DecoderSource::prepare(
    ...
    LOGI("configure decoder. surface = %p", mSurface.get());
...
Status DecoderSource::stop() {
    ...
    LOGI("Stopping decoder source..");
    mStopRequest = true;
    joinThread();

    media_status_t status = AMediaCodec_stop(mDec.get());
    if (status != AMEDIA_OK) {
        LOGE("failed to stop decoder");
    }
...
void DecoderSource::rewindExtractor() {
    ...
    if (status != AMEDIA_OK) {
        LOGE("failed to seek Extractor to 0");
    }
}
