...
Status Validator::checkOverallBitrate(const Stats &stats, const RunConfig& config) {
    ...
    if (!AMediaFormat_getInt32(config.format(), AMEDIAFORMAT_KEY_BIT_RATE, &bitrate)) {
        // should not happen
        LOGI("DEBUG: checkOverallBitrate: bitrate was not configured !");
        return FAIL;
    }
    assert(bitrate > 0);
    ...
    LOGI("RESULT: Bitrate expected=%d Achieved=%d Deviation=%.2g%%",
            bitrate, avgBitrate, deviation);

    if (fabs(deviation) > kBitrateDeviationPercentMax) {
        LOGI("RESULT: ERROR: bitrate deviation(%.2g%%) exceeds threshold (+/-%.2g%%)",
...
Status Validator::checkIntraPeriod(const Stats& stats, const RunConfig& config) {
    ...
    if (!AMediaFormat_getFloat(config.format(), AMEDIAFORMAT_KEY_FRAME_RATE, &framerate)) {
        // should not happen
        LOGI("DEBUG: checkIntraPeriod: framerate was not configured ! : %s",
    ...
    if (!AMediaFormat_getInt32(config.format(), AMEDIAFORMAT_KEY_I_FRAME_INTERVAL, &intraPeriod)) {
        // should not happen
        LOGI("DEBUG: checkIntraPeriod: I-period was not configured !");
        return OK;
    }
    ...
    if (iInterval >= stats.frameCount()) {
        LOGI("RESULT: Intra-period %d exceeds frame-count %d ..skipping",
    ...
    for (;;) {
        ...
        if (abs(nextKeyLocation - lastKeyLocation - iInterval) > kSyncFrameDeviationFramesMax) {
            LOGE("RESULT: ERROR: Intra period at frame %d is %d (expected %d +/-%d)",
    ...
    if (numGopFound) {
        LOGI("RESULT: Intra-period: configured=%d frames (%d sec). Actual=%d frames",
...
Status Validator::checkDynamicKeyFrames(const Stats& stats, const RunConfig& config) {
    LOGI("DEBUG: checkDynamicKeyFrames");
    ...
    if (config.dynamicParamsOfKind(TBD_AMEDIACODEC_PARAMETER_KEY_REQUEST_SYNC_FRAME, keyRequests) <= 0) {
        LOGI("DEBUG: dynamic key-frames were not requested");
        return OK;
    }
    ...
    for (DParamRef &d : keyRequests) {
        ...
        if (generatedKeyLocation - d->frameNum() > kSyncFrameDeviationFramesMax) {
            LOGI("RESULT: ERROR: Dynamic sync-frame requested at frame=%d, got at frame=%d",
    ...
    LOGI("RESULT: Dynamic Key-frame locations - actual/requested :");
    LOGI("RESULT:         %s", debugStr.c_str());
...
Status Validator::checkDynamicBitrate(const Stats& stats, const RunConfig& config) {
    ...
    if (!AMediaFormat_getInt32(config.format(), TBD_AMEDIAFORMAT_KEY_BIT_RATE_MODE,
        ...
        LOGI("DEBUG: checkDynamicBitrate: skipping since CBR not requested");
        return OK; //skip
    }
    ...
    if (config.dynamicParamsOfKind(TBD_AMEDIACODEC_PARAMETER_KEY_VIDEO_BITRATE, bitrateUpdates) <= 0) {
        LOGI("DEBUG: checkDynamicBitrate: dynamic bitrates not requested !");
        return OK; //skip
    }
    int32_t bitrate = 0;
    if (!AMediaFormat_getInt32(config.format(), AMEDIAFORMAT_KEY_BIT_RATE, &bitrate)) {
        // should not happen
        LOGI("DEBUG: checkDynamicBitrate: bitrate was not configured !");
        return OK; //skip
    }
    assert(bitrate > 0);
    ...
    for (DParamRef &d : bitrateUpdates) {
        ...
        if (!AMediaFormat_getInt32(
            ...
            LOGE("BUG: expected dynamic bitrate");
            continue;
        }
        ...
        if (fabs(deviation) > kBitrateDeviationPercentMax) {
            LOGI("RESULT: ERROR: dynamic bitrate deviation(%.2g%%) exceeds threshold (+/-%.2g%%)",
                    deviation, kBitrateDeviationPercentMax);
            fail |= true;
        }
    ...
    LOGI("RESULT: Dynamic Bitrates : [from-frame  -  to-frame] actual/expected :");
    LOGI("RESULT:        %s", debugStr.c_str());
